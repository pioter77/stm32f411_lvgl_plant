...
{
#if (XPT2046_USE_CS == 1)
	HAL_GPIO_WritePin(T_CS_GPIO_Port, T_CS_Pin, GPIO_PIN_RESET); // CS Active
#endif
...
#if (XPT2046_USE_CS == 1)
	HAL_GPIO_WritePin(T_CS_GPIO_Port, T_CS_Pin, GPIO_PIN_SET); // CS Idle
#endif
}
...
void XPT2046_Task(void)
{
	  ...
	  case XPT2046_PRESAMPLING:
		  ...
		  if((HAL_GetTick() - SampleTimer ) > SAMPLE_INTERVAL) // Software Timers
		  {
			  ...
			  if(GPIO_PIN_SET == HAL_GPIO_ReadPin(T_IRQ_GPIO_Port, T_IRQ_Pin))
			  {
